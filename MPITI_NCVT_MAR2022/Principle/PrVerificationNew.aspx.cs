...
public partial class Principle_PrVerificationNew : System.Web.UI.Page
{
    ...
    private void bindyear()
    {
        ...
        for (int i = 2015; i <= Convert.ToInt32(DateTime.Now.Year); i++)
        {
                ...
                if (ViewState["TotalRecords"] != null)
                {
                    ...
                    for (int i = 0; i < dtfinaltable.Rows.Count; i++)
                    {
                        if (dtfinaltable.Rows[i]["isedited"].ToString().ToLower() == "yes")
                        {
                            ...
                            affectrows = Mysql.ExecuteNonQuery("exec UpdateStudentDetails @RollNumber='" + dtfinaltable.Rows[i]["RollNumber"] + "',@Eligibility='" + dtfinaltable.Rows[i]["Eligibility"] 
                                + "',@ResultStatusED='" + dtfinaltable.Rows[i]["ResultStatusED"] + "',@ResultStatusPractical='" + dtfinaltable.Rows[i]["ResultStatusPractical"] 
                                + "',@Attendance='" + dtfinaltable.Rows[i]["Attendance"] + "',@Sessionalmarks='" + dtfinaltable.Rows[i]["Sessionalmarks"]
                            ...
                            if (!affectrows)
                            {
                                CommonUtilities.LogFileWrite("PrVerification Page Student Updation Issue:" + "exec UpdateStudentDetails @RollNumber='" + dtfinaltable.Rows[i]["RollNumber"] + "',@Eligibility='" + dtfinaltable.Rows[i]["Eligibility"]
                                + "',@ResultStatusED='" + dtfinaltable.Rows[i]["ResultStatusED"] + "',@ResultStatusPractical='" + dtfinaltable.Rows[i]["ResultStatusPractical"]
                                    + "',@Attendance='" + dtfinaltable.Rows[i]["Attendance"] + "',@Sessionalmarks='" + dtfinaltable.Rows[i]["Sessionalmarks"]
                                + "',@Paper1=" + (dtfinaltable.Rows[i]["Paper1"].ToString() == "true" ? 1 : 0) + ",@Paper2=" + (dtfinaltable.Rows[i]["Paper2"].ToString() == "true" ? 1 : 0) + ",@practical=" + (dtfinaltable.Rows[i]["practical"].ToString() == "true" ? 1 : 0)
                                + ",@Amount=" + dtfinaltable.Rows[i]["Amount"] + ",@SessionId='" + Session.SessionID + "',@PID=" + NPID + ",@ApprovalStatus='" + ApprovalStatus + "',@Paper1Status='" + dtfinaltable.Rows[i]["Paper1Status"] + "|" + dtfinaltable.Rows[i]["Paper1Header"]
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            HiddenField hdnCanResultStatusED = (HiddenField)e.Row.FindControl("hdnCanResultStatusED");
            DropDownList ddlCanResultStatusED = (DropDownList)e.Row.FindControl("ddlCanResultStatusED");
            Label lblCanResultStatusED = (Label)e.Row.FindControl("lblCanResultStatusED");

            HiddenField hdnCanResultStatusPractical = (HiddenField)e.Row.FindControl("hdnCanResultStatusPractical");
            DropDownList ddlCanResultStatusPractical = (DropDownList)e.Row.FindControl("ddlCanResultStatusPractical");
            Label lblCanResultStatusPractical = (Label)e.Row.FindControl("lblCanResultStatusPractical");
            ...
            if (hdnCanEligibility.Value != "0" && (hdnCanEligibility.Value == "discharge" || hdnCanEligibility.Value == "for verification"))
            {
                ...
                ddlCanResultStatusED.SelectedValue = hdnCanResultStatusED.Value;
                ddlCanResultStatusPractical.SelectedValue = hdnCanResultStatusPractical.Value;
                ...
                if (hdntbledited.Value.ToLower() == "yes" && hdnPaymentStatus.Value.ToLower() == "no")
                {
                    ...
                    lblCanResultStatusED.Visible = false;
                    ddlCanResultStatusED.Style.Add("display", "block");
                    lblCanResultStatusPractical.Visible = false;
                    ddlCanResultStatusPractical.Style.Add("display", "block");
                ...
                else
                {
                    ...
                    lblCanResultStatusPractical.Style.Add("font-weight", "bold");
                    lblCanResultStatusPractical.Style.Add("font-weight", "bold");
                    ...
                    if (hdnCanEligibility.Value == "for verification")
                    {
                        ...
                        lblCanResultStatusED.Visible = true;
                        lblCanResultStatusPractical.Visible = true;
                    ...
                    lblCanResultStatusED.Visible = true;
                    ddlCanResultStatusED.Style.Add("display", "none");
                    lblCanResultStatusPractical.Visible = true;
                    ddlCanResultStatusPractical.Style.Add("display", "none");
                ...
                if (ddlCanResultStatusED.SelectedValue == "Already PASS" && ddlCanResultStatusPractical.SelectedValue == "Already PASS")
                {
                    ddlCanEligibility.Enabled = false;
                }
            }
            else
            {
    ...
    private void Updateintable()
    {
        ...
        foreach (GridViewRow gvr in GridView1.Rows)
        {
            ...
            DropDownList ddlCanResultStatusED = (DropDownList)gvr.FindControl("ddlCanResultStatusED");
            DropDownList ddlCanResultStatusPractical = (DropDownList)gvr.FindControl("ddlCanResultStatusPractical");
            ...
            dtupdate.Rows[i]["ResultStatusED"] = ddlCanResultStatusED.SelectedValue;
            dtupdate.Rows[i]["ResultStatusPractical"] = ddlCanResultStatusPractical.SelectedValue;
