        ...
        if (PID > 0)
        {
            ...
            if (dtgrid != null && dtgrid.Rows.Count > 0)
            {
                ...
                if (dtgrid.Rows[0]["Paper3Status"].ToString().ToLower().Contains("yes|"))
                    GridView1.Columns[11].HeaderText = dtgrid.Rows[0]["Paper3Status"].ToString().Split('|')[1];

                GridView1.DataSource = dtgrid;
                GridView1.DataBind();
                GridViewRow head = GridView1.HeaderRow;
               // int rowcoount = GridView1.Rows.Count;

                GridViewRow row = GridView1.Rows[0];
                string addyear = row.Cells[10].Text;
                if (int.Parse("2018") >= 2018)
                {

                    head.Cells[12].Text = "Year";
                    //GridView2.Columns[9].HeaderText = "Year";
                }
                else
                    head.Cells[12].Text = "Semester";
