...
public partial class Login_LoginNew : ClsErrorLog
{
    DataTable dt;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (string.IsNullOrEmpty(Request.QueryString["username"])) {
            Response.Redirect("http://mpiti.cbtexam.in/Login/NCVTlogin.aspx");
        }
        string username = Request.QueryString["username"].ToString(); //Encrypt("PR23000571", true);
        string password = Request.QueryString["password"].ToString();//Encrypt("Little@123", true);
        txtUserName.Text = Decrypt(username, true);
        txtPassword.Text = Decrypt(password, true);
