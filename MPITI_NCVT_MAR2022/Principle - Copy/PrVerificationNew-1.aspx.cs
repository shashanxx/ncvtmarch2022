...
public partial class Principle_PrVerificationNew : System.Web.UI.Page
{
        ...
        try
        {
            if (isvalidategrid())
            {
                ...
                if (ViewState["TotalRecords"] != null)
                {
                    ...
                    try
                    {
                        var TotalStuPayment = dtfinaltable.Compute("SUM(Amount)", "Eligibility='for verification' and PaymentStatus='No'");
                    ...
                    for (int i = 0; i < dtfinaltable.Rows.Count; i++)
                    {
                        if (dtfinaltable.Rows[i]["isedited"].ToString().ToLower() == "yes")
                        {
                            ...
                            if (flag == 0)
                            {
                                if (dtfinaltable.Rows[i]["Eligibility"].ToString().ToLower() == "for verification" && dtfinaltable.Rows[i]["ChoiceLocked"].ToString().ToLower() == "yes")
                                {
    ...
    protected void BindGrid()
    {
        ...
        if (dtgrid != null && dtgrid.Rows.Count > 0)
        {
            lbltotalstucount.Text = Convert.ToInt32(dtgrid.Compute("COUNT(Eligibility)", "Eligibility='for verification' and PaymentStatus='No'")).ToString();
            try
            {
                var TotalStuPayment = dtgrid.Compute("SUM(Amount)", "Eligibility='for verification' and PaymentStatus='No'");
                if (TotalStuPayment != null && TotalStuPayment.ToString() != "")
