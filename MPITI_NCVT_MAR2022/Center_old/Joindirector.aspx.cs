...
public partial class Center_Joindirector : System.Web.UI.Page
{
    ...
    {
         if (Session["centerlogin"] == null || Session["Zone"] == null || Session["OTPValid"]=="No")
            {
        ...
        if (!IsPostBack)
        {
            ...
            { 
                HiddenField2.Value = Session["LoginId"].ToString();
    ...
    void FillGrid()
    {
        ...
        DataSet Ds = new DataSet();
        // Ds = MySql.GetDataSetWithQuery("Exec BindGridinJD '" + Actioncase + " ' , '" + session + "', '" + ITICode + "', '" + semester + "', '" + Tradecode + "' ");
        Ds = MySql.GetDataSetWithQuery("Exec BindGridinJD1 @Actioncase='" + Actioncase + "', @session='" + session + "', @ITICode='" + ITICode + "', @Semester='" + semester + "', @TradeCode='" + Tradecode + "'");
        if (Ds.Tables[0].Rows.Count > 0)
        {
                ...
                foreach (GridViewRow row1 in GridView1.Rows)
                {
                    if (row1.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        if (chkRow.Checked)
                        {
                            ...
                            //string edmarks = (row1.Cells[3].FindControl("txtedmarks") as TextBox).Text;
