...
public partial class Center_PdfJDReports_Blank : System.Web.UI.Page
{
    ...
    protected void BindOMRDetails()
    {
        ...
        Ds = MySql.GetDataSetWithQuery("Exec BindGridinJD1_BlankReports @Actioncase=" + Actioncase + ",@session='" + strsession + "',@ITICode='" + strITICode + "', @Semester='" + strSemID + "', @TradeCode='" + strTradeCode + "'");
        if (Ds.Tables[0].Rows.Count > 0)
        {
            ...
            for (int i = 0; i <= Ds.Tables[0].Rows.Count - 1; i++)
            {
             ...
             //   strData = strData + "<td style='border-bottom:1px solid; border-right:1px solid; text-align:center;'>" + Ds.Tables[0].Rows[i]["Marks"] + "</td>";
                strData = strData + "<td style='border-bottom:1px solid; border-right:1px solid; text-align:center;'></td>";
                //strData = strData + "<td style='border-bottom:1px solid; border-right:1px solid; text-align:center;'>" + Ds.Tables[0].Rows[i]["PresentAbsent"] + "</td>";
                strData = strData + "<td style='border-bottom:1px solid; border-right:1px solid; text-align:center;'></td>";
